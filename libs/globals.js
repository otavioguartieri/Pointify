...
function modal(act, content, confirm = {}, cancel = {}) {
    ...
    const defaultCancel = { text: 'Cancelar', action: '', close: true, center: false };
    const defaultConfirm = { text: 'Confirmar', action: '', close: true, center: false };

    cancel = { ...defaultCancel, ...cancel };
    ...
    $('body').append(`
        ...
        <div style="z-index: ${actualTime}9;" id="modal${actualTime}" class="modal">
            <div class="modalContent">
                ...
                <div class="actions">
                    <button class="cancel" onclick="${cancel['action']}$('#modalOverlay${actualTime}').remove(); $('#modal${actualTime}').remove();">${cancel['text']}</button>
                    <button class="confirm" onclick="${confirm['action']}">${confirm['text']}</button>
                </div>
            </div>
        </div>
    `);

    if(act[0] != 'task' && confirm['close'] == true){
        $(`#modal${actualTime} .modalContent .actions button.confirm`)
            .on('click', function () {
                $(`#modalOverlay${actualTime}`).remove();
                $(`#modal${actualTime}`).remove();
            });
    }
    ...
    if(cancel['close'] == true){
        $(`#modal${actualTime} .modalContent .actions button.cancel`)
            .on('click', function () {
                $(`#modalOverlay${actualTime}`).remove();
                $(`#modal${actualTime}`).remove();
            });
